...
<!-- Asegúrate de incluir la librería Animate.css para la animación -->
...
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css">
    ...
    <h4 class="blog-card-caption">
        <a href="#" data-toggle="modal" data-target="#myModal" data-post-id="<?php echo $p->id; ?>" data-post-token="<?php echo generar_token($p->id); ?>">
            <?php echo $p->id.".-"; ?> Lorem Ipsum is simply dummy text of the printing and
        </a>
    </h4>
    <div class="ftr">
        <div class="author">
            <a href="#" data-toggle="modal" data-target="#myModal" data-post-id="<?php echo $p->id; ?>" data-post-token="<?php echo generar_token($p->id); ?>">
                <img src="https://media.licdn.com/dms/image/C5612AQFqAiEvyP5sMQ/article-cover_image-shrink_600_2000/0/1520248896936?e=2147483647&v=beta&t=xhceNaDz3y_eD_GRjSeX4W5cpi6yS_ckdYgLiNMQUd0" alt="..." class="avatar img-raised">
                <span>Postular</span>
            </a>
        </div>
    </div>
...
<!-- Modal -->
<div class="modal fade" data-backdrop="static" id="myModal" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <!-- Utiliza la clase modal-lg para un modal más grande -->
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myModalLabel">Enlace Generado</h5>
            ...
            <div class="modal-body">
                ...
                <div class="input-group mb-3">
                        <input type="text" id="miInput" disabled class="form-control" placeholder="Recipient's username" aria-label="Recipient's username" aria-describedby="button-addon2">
                            <div class="input-group-append">
                            <button  button class="btn btn-outline-secondary" type="button" id="button-addon2">
                            <i id="copiedIcon" class="far fa-copy"></i> <span id="textoBoton">Copiar</span>
                        </button>
                    </div>
                </div>
            </div>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-Fy6S3B9q64WdZWQUiU+q4/2Lc9npb8tCaSX9FK7E8HnRr0Jz8D6OP9dO5Vg3Q9ct" crossorigin="anonymous"></script>
    ...
    $('#myModal').on('show.bs.modal', function (event) {
        // Llamada a tu función para obtener el texto deseado
        // Obtener el enlace que abrió el modal
        var link = $(event.relatedTarget);

// Obtener el ID y el token de los atributos de datos del enlace
        var postId_list = link.data('post-id');
        var postToken = link.data('post-token');
        var texto = obtenerTexto();

        // Actualizar el valor del input en el modal con el texto obtenido
        $('#miInput').val('http://localhost/codeigniter/form/'+postId_list+'/'+postToken);
        var formData = new FormData();
                    formData.append("postId_list", postId_list);
                    formData.append("token", postToken);

                $.ajax({
                        type: "POST",
                        url: "<?php echo site_url('Vista/guardarToken'); ?>",
                        data: formData,
                        contentType: false,
                        processData: false,
    ...
    // Función que simula obtener un texto (puedes reemplazarla con tu lógica real)
    function obtenerTexto() {
        // Aquí puedes realizar cualquier lógica para obtener el texto dinámicamente
        // Por ejemplo, podrías hacer una solicitud AJAX para obtener datos del servidor
        // En este caso, la función simplemente retorna un texto fijo
        return "Texto obtenido dinámicamente";
    }
</script>
        <script>
    ...
    $(document).ready(function () {
        $('#button-addon2').on('click', function () {
            // Cambiar la animación del icono (puedes ajustar la clase según tus necesidades)
            // Cambiar la animación del icono (puedes ajustar la clase según tus necesidades)
            var contenidoACopiar = $('#miInput').val();
            navigator.clipboard.writeText(contenidoACopiar).then(function() {
            console.log('Texto copiado al portapapeles:', contenidoACopiar);
            ...
            $('#copiedIcon').removeClass('far fa-copy').addClass('fas fa-check-circle animate__animated animate__zoomIn');
            $('#textoBoton').text('Copiado');

            // Después de un tiempo, restablecer la animación y ocultar el badge
            setTimeout(function () {
                // Restablecer la animación del icono
                $('#textoBoton').text('Copiar');
                $('#copiedIcon').removeClass('fas fa-check-circle animate__animated animate__zoomIn').addClass('far fa-copy');
            }, 1050); // Ajusta el tiempo según tus necesidades
        });
    });
</script>
    </body>
